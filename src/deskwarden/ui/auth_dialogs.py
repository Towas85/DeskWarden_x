...
def show_global_unlock_auth(on_success):
    """
    Zobrazí autorizačný dialóg pre globálne odomknutie ochrany DeskWarden.
    Po úspešnom zadaní hesla zavolá callback on_success().
    """
    global _unlock_dlg_ref
    if _unlock_dlg_ref is not None:
        try:
            if _unlock_dlg_ref.isVisible():
                _unlock_dlg_ref.activateWindow()
                _unlock_dlg_ref.raise_()
                return
        except Exception:
            _unlock_dlg_ref = None
    ...
    tb_logo.setFixedSize(16, 16)
    from PyQt6.QtGui import QPixmap
    _ub_pm = QPixmap(asset_path("icon.png"))
    if not _ub_pm.isNull():
        _ub_pm = _ub_pm.scaled(16, 16, Qt.AspectRatioMode.KeepAspectRatio,
                                Qt.TransformationMode.SmoothTransformation)
        tb_logo.setPixmap(_ub_pm)
    tb_logo.setStyleSheet("background: transparent;")
    tb_logo.setContentsMargins(3, 2, 0, 0)
    tbl.addWidget(tb_logo)
    tbl.addSpacing(6)

    tl = QLabel("DeskWarden  ·  Unlock Protection")
    tl.setFont(QFont("Segoe UI", 8))
    tl.setStyleSheet(f"color: {_MUTE}; background: transparent;")
    tbl.addWidget(tl)
    tbl.addStretch()
    ...
    icon_lbl = QLabel("🔓")
    icon_lbl.setFont(QFont("Segoe UI Emoji", 22))
    icon_lbl.setStyleSheet(f"color: {_ACC2}; background: transparent;")
    icon_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
    sbl.addWidget(icon_lbl)
    ...
    prompt = QLabel("Enter password to unlock protection")
    prompt.setFont(QFont("Segoe UI", 10))
    prompt.setStyleSheet(f"color: {_FG}; background: transparent;")
    rpl.addWidget(prompt)

    sub = QLabel("Apps will be resumed and DeskWarden stays active.")
    sub.setFont(QFont("Segoe UI", 8))
    sub.setStyleSheet(f"color: {_MUTE}; background: transparent;")
    rpl.addWidget(sub)
    ...
    u_eye_btn = QPushButton("Show", pw_edit)
    u_eye_btn.setFixedSize(34, 22)
    u_eye_btn.setCursor(QCursor(Qt.CursorShape.PointingHandCursor))
    u_eye_btn.setStyleSheet(f"""
        QPushButton {{ background: transparent; color: {_MUTE};
            border: none; font-size: 7.5pt; font-weight: 600; }}
        QPushButton:hover {{ color: #ede9ff; }}""")
    _u_pw_visible = [False]
    def _u_toggle_eye():
        _u_pw_visible[0] = not _u_pw_visible[0]
        pw_edit.setEchoMode(QLineEdit.EchoMode.Normal if _u_pw_visible[0]
                            else QLineEdit.EchoMode.Password)
        u_eye_btn.setText("Hide" if _u_pw_visible[0] else "Show")
    u_eye_btn.clicked.connect(_u_toggle_eye)

    def _pos_u_eye():
        u_eye_btn.move(pw_edit.width() - u_eye_btn.width() - 6,
                       (pw_edit.height() - u_eye_btn.height()) // 2)
    _orig_pw_resize = pw_edit.resizeEvent
    def _pw_resize(ev):
        _orig_pw_resize(ev)
        _pos_u_eye()
    ...
    acc_line = QFrame()
    acc_line.setFixedHeight(2)
    acc_line.setStyleSheet(f"background: {_ACC}; border-radius: 1px;")
    rpl.addWidget(acc_line)

    err_lbl = QLabel("")
    _err_font = QFont("Segoe UI", 9)
    _err_font.setFamilies(["Segoe UI", "Segoe UI Symbol"])
    err_lbl.setFont(_err_font)
    err_lbl.setStyleSheet(f"color: {_RED}; background: transparent;")
    rpl.addWidget(err_lbl)

    unlock_btn = QPushButton("🔓  Unlock Protection")
    unlock_btn.setFixedHeight(40)
    unlock_btn.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
    unlock_btn.setCursor(QCursor(Qt.CursorShape.PointingHandCursor))
    unlock_btn.setStyleSheet(f"""
    ...
    # ── Forgot password link ───────────────────────────────────────────────
    forgot_row_u = QWidget()
    forgot_row_u.setFixedHeight(20)
    forgot_row_u.setStyleSheet("background: transparent;")
    frl_u = QHBoxLayout(forgot_row_u)
    frl_u.setContentsMargins(0, 0, 0, 0)
    frl_u.setSpacing(0)
    frl_u.addStretch()

    forgot_btn_u = QPushButton("Forgot password?")
    forgot_btn_u.setFont(QFont("Segoe UI", 8))
    forgot_btn_u.setCursor(QCursor(Qt.CursorShape.PointingHandCursor))
    forgot_btn_u.setAutoDefault(False)
    forgot_btn_u.setDefault(False)
    forgot_btn_u.setStyleSheet(f"""
        QPushButton {{ background: transparent; color: {_MUTE}; border: none; }}
        QPushButton:hover {{ color: {_ACC2}; text-decoration: none; }}
    """)

    def _open_recovery_u():
        try:
            from .recovery_dialog import show_recovery_modal
            def _on_rec_success_u():
                _unlocked_u[0] = True
                dlg.close()
                on_success()
            def _on_rec_close_u():
                if not _unlocked_u[0]:
                    dlg.raise_()
                    dlg.activateWindow()
                    pw_edit.setFocus()
            rec = show_recovery_modal(on_success=_on_rec_success_u, on_close=_on_rec_close_u, source_context="global_unlock", parent=None)
            if not rec:
                dlg.raise_()
                dlg.activateWindow()
        except Exception as e:
            dlg.raise_()
            dlg.activateWindow()
            log_crash("show_global_unlock_auth/_open_recovery_u", e)

    forgot_btn_u.clicked.connect(_open_recovery_u)
    frl_u.addWidget(forgot_btn_u)
    frl_u.addStretch()

    try:
        from .recovery_dialog import is_recovery_available
        if not is_recovery_available(load_config()):
            forgot_btn_u.setVisible(False)
    except Exception:
        pass

    rpl.addWidget(forgot_row_u)
    ...
    def _on_txt_changed_u(t):
        if err_lbl.text() == "Please enter your password.":
            err_lbl.setText("")
    pw_edit.textChanged.connect(_on_txt_changed_u)
    ...
    def _start_cd(seconds):
        ...
        def _tick():
            ...
            if r <= 0:
                ...
                unlock_btn.setText("🔓  Unlock Protection")
                err_lbl.setText("")
                pw_edit.setFocus()
                return
            err_lbl.setText(f"🔒︎ Locked — try again in {r}s")
    ...
    def _attempt():
        if _attempting_u[0] or _unlocked_u[0]:
            return
        _attempting_u[0] = True
        try:
            entered_txt = pw_edit.text()
            if not entered_txt:
                err_lbl.setText("Please enter your password.")
                pw_edit.setFocus()
                return
            is_locked, wait_s = check_locked_out(CTX_U)
            if is_locked:
                _start_cd(wait_s)
                return
            current_pw_hash = load_config().get("password_hash", cfg.get("password_hash", ""))
            if hash_pw(entered_txt) == current_pw_hash:
                _unlocked_u[0] = True
                reset_attempt_state(CTX_U)
                log_security_event("success", CTX_U, "global unlock confirmed")
                try:
                    from .recovery_dialog import close_active_recovery_dialog
                    close_active_recovery_dialog()
                except Exception:
                ...
                state = record_wrong_attempt(CTX_U)
                pw_edit.clear()
                if state["locked"]:
                    _start_cd(state["wait"])
                else:
    ...
    _lk, _ws = check_locked_out(CTX_U)
    if _lk:
        _start_cd(_ws)
